...
Tratar esta Skill como módulo de permissões e segurança funcional. Não atuar como fonte independente de regra de negócio e não produzir uma Modelagem Funcional final concorrente com a `@beta`.
...
Quando clique repetido, duplicidade ou persistência dependerem de processamento especializado, devolver o ponto à `@beta` para composição com `@beta-mod-processamento`.
